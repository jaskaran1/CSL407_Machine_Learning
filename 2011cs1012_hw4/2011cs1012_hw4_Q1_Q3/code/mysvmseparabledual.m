function alpha=mysvmseparabledual(X,Y,K)
...
Aeq=ones(n,1)*Y';
lb=zeros(n,1);
%opts=optimset('Algorithm','interior-point');%Display iter doesn't work using optimset
%opts=optimset('MaxIter',10);
opts=optimoptions('quadprog','Algorithm','interior-point-convex');
[alpha,fval,exitflag,output]=quadprog(H,f,[],[],Aeq,beq,lb,[],[],opts);
fval
exitflag
output
end